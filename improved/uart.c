#include "device_driver.h"
#include "app_state.h"
#include <stdio.h>
#include <stdarg.h>
#include <string.h>

void Uart2_Init(int baud)
{
    double div; unsigned int mant, frac;
    Macro_Set_Bit(RCC->AHB1ENR, 0);
    Macro_Set_Bit(RCC->APB1ENR, 17);
    Macro_Write_Block(GPIOA->MODER, 0xf, 0xa, 4);
    Macro_Write_Block(GPIOA->AFR[0], 0xff, 0x77, 8);
    Macro_Write_Block(GPIOA->PUPDR, 0xf, 0x5, 4);

    div = PCLK1 / (16. * baud);
    mant = (int)div;
    frac = (int)((div - mant) * 16. + 0.5);
    mant += frac >> 4; frac &= 0xf;
    USART2->BRR = (mant << 4) | (frac << 0);
    USART2->CR1 = (1 << 13) | (1 << 3) | (1 << 2);
    USART2->CR2 = 0;
    USART2->CR3 = 0;
}

void Uart2_Send_Byte(char data)
{
    if (data == '\n') { while (!Macro_Check_Bit_Set(USART2->SR, 7)); USART2->DR = 0x0d; }
    while (!Macro_Check_Bit_Set(USART2->SR, 7));
    USART2->DR = data;
}

void Uart2_Send_String(char *pt) { while (*pt) Uart2_Send_Byte(*pt++); }

void Uart2_Printf(char *fmt, ...)
{
    va_list ap; char s[256];
    va_start(ap, fmt); vsprintf(s, fmt, ap); Uart2_Send_String(s); va_end(ap);
}

void Uart1_Init(int baud)
{
    double div; unsigned int mant, frac;
    Macro_Set_Bit(RCC->AHB1ENR, 0);
    Macro_Set_Bit(RCC->APB2ENR, 4);
    Macro_Write_Block(GPIOA->MODER, 0xf, 0xa, 18);
    Macro_Write_Block(GPIOA->AFR[1], 0xff, 0x77, 4);
    Macro_Write_Block(GPIOA->PUPDR, 0xf, 0x5, 18);

    div = PCLK2 / (16. * baud);
    mant = (int)div;
    frac = (int)((div - mant) * 16 + 0.5);
    mant += frac >> 4; frac &= 0xf;
    USART1->BRR = (mant << 4) | (frac << 0);
    USART1->CR1 = (1 << 13) | (1 << 5) | (1 << 3) | (1 << 2);
    USART1->CR2 = 0;
    USART1->CR3 = 0;

    NVIC_EnableIRQ(USART1_IRQn);
}

void Uart1_Send_Byte(char data)
{
    if (data == '\n') { while (!Macro_Check_Bit_Set(USART1->SR, 7)); USART1->DR = 0x0d; }
    while (!Macro_Check_Bit_Set(USART1->SR, 7));
    USART1->DR = data;
}

void Uart1_Send_String(char *pt) { while (*pt) Uart1_Send_Byte(*pt++); }

void Uart1_Printf(char *fmt, ...)
{
    va_list ap; char s[256];
    va_start(ap, fmt); vsprintf(s, fmt, ap); Uart1_Send_String(s); va_end(ap);
}

volatile char uart1_buffer[64];
static int uart1_rx_index = 0;
volatile int uart1_rx_exist = 0;
volatile char uart2_buffer[64];
volatile int  uart2_rx_exist = 0;

void USART1_IRQHandler(void)
{
    if (USART1->SR & (1 << 5)) {
        char ch = (char)(USART1->DR & 0xFF);

        if (uart1_rx_exist == 1) return;

        if (ch == '\r' || ch == '\n') {
            uart1_buffer[uart1_rx_index] = '\0';
            uart1_rx_index = 0;
            if (uart1_buffer[0] != '\0') uart1_rx_exist = 1;
        } else if (uart1_rx_index < 63) {
            uart1_buffer[uart1_rx_index++] = ch;
        }
    }
}

void Uart2_RX_Interrupt_Enable(void)
{
    USART2->CR1 |= (0x1 << 5);
    NVIC_ClearPendingIRQ(USART2_IRQn);
    NVIC_EnableIRQ(USART2_IRQn);
}
