...
// LCD I2C 주소(LCD1_ADDR / LCD2_ADDR)는 pin_map.h 에서 관리한다.

// I2C 통신 설정 (PB8: SCL, PB9: SDA)
void I2C1_Init(void) {
    // 1. 클럭 활성화 (GPIOB, I2C1)
    Macro_Set_Bit(RCC->AHB1ENR, PIN_I2C_RCC_BIT);
    Macro_Set_Bit(RCC->APB1ENR, 21); 

    // 2. PB8, PB9 핀 설정 (Alternate Function, Open-Drain)
    Macro_Write_Block(PIN_I2C_PORT->MODER, 0x3, 0x2, PIN_POS2(PIN_I2C_SCL_NUM));
    Macro_Write_Block(PIN_I2C_PORT->MODER, 0x3, 0x2, PIN_POS2(PIN_I2C_SDA_NUM));
    Macro_Set_Bit(PIN_I2C_PORT->OTYPER, PIN_I2C_SCL_NUM);   // Open Drain (I2C 필수)
    Macro_Set_Bit(PIN_I2C_PORT->OTYPER, PIN_I2C_SDA_NUM);
    Macro_Write_Block(PIN_I2C_PORT->PUPDR, 0x3, 0x1, PIN_POS2(PIN_I2C_SCL_NUM));
    Macro_Write_Block(PIN_I2C_PORT->PUPDR, 0x3, 0x1, PIN_POS2(PIN_I2C_SDA_NUM));
    Macro_Write_Block(PIN_I2C_PORT->AFR[PIN_AFR_IDX(PIN_I2C_SCL_NUM)], 0xF, PIN_I2C_AF, PIN_AFR_POS(PIN_I2C_SCL_NUM));
    Macro_Write_Block(PIN_I2C_PORT->AFR[PIN_AFR_IDX(PIN_I2C_SDA_NUM)], 0xF, PIN_I2C_AF, PIN_AFR_POS(PIN_I2C_SDA_NUM));
